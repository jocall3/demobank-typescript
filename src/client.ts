...
export interface ClientOptions {
  /**
   * API Key authentication for server-to-server and trusted developer applications, typically used for developer tools and webhook validation.
   */
  apiKey?: string | null | undefined;

  /**
   * Bearer token authentication validated against biometric enrollment, used for high-security transactions or sensitive data access.
   */
   ...
   *
   * Defaults to process.env['DEMOBANK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DEMOBANK_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Demobank API.
 */
export class Demobank {
   ...
   * API Client for interfacing with the Demobank API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['DEMOBANK_API_KEY'] ?? null]
   * @param {string | null | undefined} [opts.bearerToken=process.env['DEMOBANK_BEARER_TOKEN'] ?? null]
   * @param {string} [opts.baseURL=process.env['DEMOBANK_BASE_URL'] ?? /] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEMOBANK_BASE_URL'),
    apiKey = readEnv('DEMOBANK_API_KEY') ?? null,
    bearerToken = readEnv('DEMOBANK_BEARER_TOKEN') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Demobank.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DEMOBANK_LOG'), "process.env['DEMOBANK_LOG']", this) ??
      defaultLogLevel;
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.DemobankError(
  ...
  static Demobank = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DemobankError = Errors.DemobankError;
...
Demobank.Users = Users;
Demobank.Accounts = Accounts;
Demobank.Transactions = Transactions;
Demobank.AI = AI;
Demobank.Corporate = Corporate;
Demobank.Web3 = Web3;
Demobank.Payments = Payments;
Demobank.Sustainability = Sustainability;
Demobank.Lending = Lending;
Demobank.Developers = Developers;
Demobank.Identity = Identity;

export declare namespace Demobank {
